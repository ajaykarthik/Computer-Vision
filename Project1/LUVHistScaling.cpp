...
Descripiton: This program gets an input color image, performs histogram scaling in the Luv domain on the given window
The conversion matrix is calcualted on the given window
histogram equalization is performed on the whole image
...
void runOnWindow(int W1, int H1, int W2, int H2, Mat inputImage, char *outName) {
	...
	int luminenceHistogram[101];
	int summationf[101];// summation f(i)
	int conversionTable[101];
	for (int i = 0; i < 101; i++)
	{
		luminenceHistogram[i] = 0;
		summationf[i] = 0;
		conversionTable[i] = 0;
	}
	for (int i = H1; i <= H2; i++)
	for (int j = W1; j <= W2; j++) {
		...
		luminenceHistogram[(int)(L + 0.5)]++;
	}
	//compute summation f(i)
	summationf[0] = luminenceHistogram[0];
	for (int i = 1; i < 101; i++)
	{
		summationf[i] = luminenceHistogram[i] + summationf[i - 1];
	}
	//create the conversion table

	int rangeOfValues = 101;
	int numPixels = summationf[100];
	//compute conversion table
	conversionTable[0] = summationf[0] * (double)rangeOfValues / (2.0*numPixels);
	for (int i = 1; i < 101; i++)
	{
		conversionTable[i] = (summationf[i - 1] + summationf[i])*(double)rangeOfValues / (2.0*numPixels);
	}
...
int main(int argc, char** argv) {
	...
	cv::namedWindow(windowInput, CV_WINDOW_AUTOSIZE);
	cv::imshow(windowInput, inputImage);
